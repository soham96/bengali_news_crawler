...
def get_all_links():
    ...
    for d in tqdm(data):
        ...
        if not LINKS:
            LINKS=['https://ebela.in/search-results-page/search-7.519094?q=world&fromSearch=yes&page=1&short=desc&slab=0&tnp=59'.replace('world', d)]
        VISITED_LINKS=get_visited_links()

        total_links=0
        pbar = tqdm(total=1000000)
        while len(VISITED_LINKS)<1000000:
